...
def load_stock_list(url="https://raw.githubusercontent.com/therkut/bistLists/refs/heads/main/data/stock_xktum_data.csv"):
    """GitHub üzerinden katılım endeksi (XKTUM) hisse listesini çeker."""
    try:
        ...
        # Hisseleri büyük harfe çevirerek ve temizleyerek listeye alıyoruz
        stock_list = df['stock'].dropna().astype(str).str.strip().str.upper().tolist()
        print(f"Katılım endeksi listesi yüklendi: {len(stock_list)} hisse.")
        return stock_list
...
def scrape_data(url, is_cmi_mode=False):
    """Web sitesinden sağlanan HTML tablo yapısına göre verileri çeker ve katılım filtresi uygular."""
    if not url:
        return []
    ...
    # 3 İş günü filtresi (Haftasonlarını atlar)
    ...
    # Katılım listesini her durumda yüklüyoruz
    STOCK_LIST = load_stock_list()
    
    if not STOCK_LIST:
        print("Kritik Uyarı: Katılım listesi boş veya yüklenemedi. Filtreleme yapılamıyor!")
        return []
    ...
    for row in rows:
        ...
        # Hisse adını temizle (BIST: ekini kaldır ve büyük harf yap)
        stock_raw = cols[0].get_text(strip=True).upper()
        stock = stock_raw.replace("BIST:", "").strip()

        # Tablo yapısına göre sütunları oku
        if is_cmi_mode:
            # CMI yapısı: Hisse, Sinyal, Cmi, Cmf, Tarih
            support_price = "-"
            ...
            # Agresif yapısı: Hisse, Destek, Sinyal, Cmi, Tarih
        ...
        try:
            ...
            if date_obj >= three_business_days_ago:
                # KATILIM FİLTRESİ (Her durumda zorunlu)
    ...
    if filtered_out_stocks:
        print(f"Bilgi: {len(filtered_out_stocks)} hisse katılım dışı olduğu için elendi: {', '.join(set(filtered_out_stocks))}")
...
def send_email(stock_signals, from_name, from_address, to_addresses, password, subject, report_title, is_cmi_mode=False, smtp_server="smtp.gmail.com", smtp_port=465):
    """Filtrelenmiş sinyalleri HTML formatında e-posta gönderir."""
    if not stock_signals:
        print(f"{report_title} için uygun sinyal yok (Filtreler nedeniyle boş).")
        return
    ...
    <div style="max-width:650px; margin:0 auto; background-color: #ffffff; border-radius:12px; overflow:hidden; box-shadow: 0 4px 10px rgba(0,0,0,0.1);">
        <div style="background: linear-gradient(135deg, #1e3c72 0%, #2a5298 100%); color:white; padding:30px; text-align:center;">
            ...
            <p style="margin:10px 0 0 0; opacity: 0.8;">{date_str} Günlük Rapor</p>
        </div>
    ...
    for i, signal in enumerate(stock_signals):
        ...
        row_content = f"""
            ...
            <td style="padding:12px; text-align:center; color: #27ae60; font-weight: 600;">{signal['signal_price']}</td>
    ...
    html_body += f"""
        ...
        <div style="background-color:#fff3cd; padding:15px; text-align:center; font-size:12px; color:#856404;">
            <strong>⚠️ YASAL UYARI:</strong> Yatırım tavsiyesi değildir. Katılım endeksine göre filtrelenmiştir.
        </div>
    ...
    try:
        ...
        print(f"{report_title} e-postası gönderildi!")
    except Exception as e:
        print(f"Hata: {e}")
...
if __name__ == "__main__":
    ...
    if not all([email_user, email_pass, to_email]):
        print("Hata: Temel kimlik bilgileri eksik.")
    else:
        to_email_list = [e.strip() for e in to_email.split(',')]
        sender_name = "Magnus Trade"
        
        # 1. Agresif Sinyal Taraması
        if url_agresif:
            print("Agresif tarama başlıyor...")
            res_agresif = scrape_data(url_agresif, is_cmi_mode=False)
            send_email(res_agresif, sender_name, email_user, to_email_list, email_pass, 
                       "📊 Agresif Hisse Sinyal Raporu", "Agresif Hisse Sinyalleri", is_cmi_mode=False)

        # 2. CMI Sinyal Taraması
        if url_cmi:
            print("\nCMI/CMF tarama başlıyor...")
            res_cmi = scrape_data(url_cmi, is_cmi_mode=True)
            send_email(res_cmi, sender_name, email_user, to_email_list, email_pass, 
